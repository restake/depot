...
async function computeResponse(request: IncomingRequestCf, env: WorkerEnv): Promise<Response> {
    ...
    if (!internalKeys.has(key)) {
        ...
        if (obj) {
            ...
            const accessControlAllowOrigin = computeAccessControlAllowOrigin(
                obj,
...
function computeObjResponse(
    obj: R2Object,
    status: number,
    opts: { range?: R2Range; onlyIf?: R2Conditional; accessControlAllowOrigin?: string; disableRangeRequests?: boolean } = {},
): Response {
    const { onlyIf } = opts;
...
function computeHeaders(
    obj: R2Object,
    opts: { range?: R2Range; accessControlAllowOrigin?: string; disableRangeRequests?: boolean },
): Headers {
...
function computeAccessControlAllowOrigin(
    obj: R2Object,
    requestOrigin: string | undefined,
    allowCorsOrigins: Set<string>,
    allowCorsTypes: Set<string>,
): string | undefined {
    // is request origin allowed?
    const allowedOrigin = allowCorsOrigins.size === 0
        ? undefined
        : allowCorsOrigins.has("*")
        ? "*"
        : requestOrigin && allowCorsOrigins.has(requestOrigin)
        ? requestOrigin
        : undefined;
    ...
    return allowCorsTypes.size === 0 ||
            ...
            allowCorsTypes.has(contentType)
        ? allowedOrigin
        : undefined;
}
